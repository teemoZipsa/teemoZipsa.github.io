 ...
 * sw.js — 오프라인 지원 + 캐싱
 */
const CACHE_PREFIX = 'teemozipsa-shell-';
const CACHE_NAME = CACHE_PREFIX + 'v1.3';
const LEGACY_CACHE_NAMES = ['teemozipsa-v1.2'];
    ...
    caches.keys().then(function(cacheNames) {
      return Promise.all(
        cacheNames.filter(function(name) {
          return name !== CACHE_NAME && (name.startsWith(CACHE_PREFIX) || LEGACY_CACHE_NAMES.includes(name));
        }).map(function(name) {
...
self.addEventListener('fetch', function(event) {
  ...
  function matchOwnedCache(targetRequest) {
    return caches.open(CACHE_NAME).then(function(cache) {
      return cache.match(targetRequest);
    });
  }
  ...
  if (request.headers.get('Accept') && request.headers.get('Accept').includes('text/html')) {
    event.respondWith(
      fetch(request).then(function(response) {
        ...
        // 오프라인이면 캐시에서 제공
        return matchOwnedCache(request).then(function(cached) {
          return cached || new Response(
            ...
            '<br><a href="/special-chars/" style="color:#a78bfa;text-decoration:underline">도구 모음으로 돌아가기</a></div></body></html>',
  ...
  // 정적 리소스: Cache First 
  event.respondWith(
    matchOwnedCache(request).then(function(cached) {
      if (cached) {
